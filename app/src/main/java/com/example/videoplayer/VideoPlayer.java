...
public class VideoPlayer implements SurfaceHolder.Callback, SeekBar.OnSeekBarChangeListener{
    ...
    VideoPlayer(SurfaceView aSurfaceView, SeekBar aSeekBar, String aPath){
...
//        if((videoStatus == VIDEO_PLAY || videoStatus == VIDEO_PAUSE) && fromUser)
//            videoSeekTo(progress);
    ...
    public void onStopTrackingTouch(SeekBar seekBar) {
        if((videoStatus == VIDEO_PLAY || videoStatus == VIDEO_PAUSE))
            videoSeekTo(seekBar.getProgress());// 6,144,233,84,76,93,85
    }
    ...
    private void videoSeekTo(int progress){
        videoThread.seekTo(progress);
        audioThread.seekTo(progress);
    }
