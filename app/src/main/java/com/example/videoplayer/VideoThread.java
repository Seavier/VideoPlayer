...
public class VideoThread extends Thread {
    ...
    public void run() {
        ...
        while(!Thread.interrupted()){
            ...
            if(isInput) {
                if(isSeek){
                    videoExtractor.seekTo(newTime * 1000, MediaExtractor.SEEK_TO_PREVIOUS_SYNC);
                    Log.e(TAG, "newTime" + newTime);
                    Log.e(TAG, "videoExtractor.getSampleTime()" + videoExtractor.getSampleTime() / 1000);
                    videoCodec.flush();
                    firstOutput = true;
                    synchronized (this){
                        isSeek = false;
                ...
                if(inputIndex >= 0){
                    ...
                    if(bufferSize >= 0){
                        if(videoExtractor.getSampleFlags() == MediaExtractor.SAMPLE_FLAG_SYNC)
                            Log.e(TAG, "关键帧！！！" + videoExtractor.getSampleTime() / 1000000);
                    ...
                    try{
                        ...
                        if(newTime >= 0 && presentationTime > newTime){
                            if(firstOutput){
                                ...
                                AudioThread.cancelPauseForVideoThread();
                            }
