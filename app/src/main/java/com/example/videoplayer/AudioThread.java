...
public class AudioThread extends Thread {
    ...
    private static boolean PAUSE_FOR_VIDEO_THREAD;
        ...
        while(!Thread.interrupted()){
            ...
            if(isInput){
                if(isSeek){
                    ...
                    synchronized (this){
                        isSeek = false;
                        PAUSE_FOR_VIDEO_THREAD = true;
                        pauseTime = 0;
                        // 防止视频线程先解码出第一个可播放缓冲区后，通过getCurrentPosition方法得到一个拖动滑块之前的、未更新的currentPosition
                        // 这样会导致视频线程休眠时间错误无法正确播放视频帧
...
//                        long sleepTime = presentationTime - mAudioTrack.getPlaybackHeadPosition() * 1000 / mSampleRate;
    ...
    public static void cancelPauseForVideoThread(){
        PAUSE_FOR_VIDEO_THREAD = false;
    }
}
